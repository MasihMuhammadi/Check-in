...
const Courses = () => {
  ...
  const searchCourse = () => {
    setIsSearched(true);
    const result = courseData.filter((course: any) =>
  ...
  const coursesToDisplay =
    searchedCourse.length > 0 ? searchedCourse : courseData;
      ...
      <div className="mb-6 grid grid-cols-1 gap-x-5 sm:gap-x-1 sm:grid-cols-2 md:grid-cols-3 md:gap-x-6 lg:grid-cols-4 xl:grid-cols-4 items-center content-center justify-center gap-4">
        {isLoading ? (
          Array.from({ length: 8 }).map((_, id) => (
            <div
              key={id}
              className="border border-gray-400 rounded-2xl px-1 mb-5 "
            >
              <Skeleton
                borderRadius={10}
                baseColor="#f0f0f0"
          ...
          <>
            ...
            {coursesToDisplay.length > 0 ? (
              coursesToDisplay.map((course: any, index: number) => (
                <div key={index} className="flex justify-center items-center">
                  <div className="border border-black w-[300px] h-80 rounded-xl text-blue-500">
                    ...
                    <div className="px-4">
                      <p className="text-black text-center font-medium text-xl capitalize mt-2">
                        {course.courseName}
                      </p>
                      <Buttons
                        secondary={true}
                        style="px-5 text-black mt-4 m-auto"
                      >
              ...
              <div className="relative">
                <NoCourseFound
                  className={"w-full sm:w-[500px] opacity-50 m-auto"}
                />
              </div>
            )}
          </>
