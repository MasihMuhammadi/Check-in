...
const SingleClass = ({ params }: { params: any }) => {
  ...
  const showFullModal = useSelector(
    (state: any) => state.studentSlice.showFullModal
  );
  const isEditable = useSelector((state: any) => state.studentSlice.isEditable);
  const pageWillShow = useSelector(
    (state: any) => state.studentSlice.pageWillShow
  );
  const singleStudent = useSelector(
    (state: any) => state.studentSlice.singleStudent
  );
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      const response: any = await axios.get(`${baseUrl}/api/students/student`);
      const newStudent = response?.data?.filter(
        (s: any) => s?.class_name === params?.classHandle?.split("-").join(" ")
      );
  ...
  const handleDelete = async () => {
    await axios.delete(`${baseUrl}/api/students/student/${clickedId}`, {
  ...
  const sendEmail = async () => {
    ...
    if (currentStatus == "present") {
      await axios.put(
        `${baseUrl}/api/students/student/${data?._id}`,
        {
    ...
    const payload = {
      ...
      html: `
            <p>
                    ...
                    We are writing to inform you that your child, <b>${
                      data?.name
                    }</b>, was
                    <span style="color:${
                      ...
                      data?.class_name
                    }</b> class. If you have any questions or concerns regarding your child's attendance or anything else related to their education,<br />
                ...
                <p>
                    ...
                    ما می‌خواهیم به شما اطلاع دهیم که فرزندتان، <b>${
                      data?.name
                    }</b>،
                    <span style="color:${
                      status == "present" ? "green" : "red"
                    ...
                    در کلاس <b>${
                      data?.class_name
                    }</b> امروز بود. اگر سوال یا نگرانی‌ای در مورد حضور فرزندتان یا هر موضوع دیگری مرتبط با تحصیلات او دارید،<br />
  ...
  const generatePDF = async (cls: any) => {
    ...
    doc.text(
      "Dear Parent, We are pleased to provide you with the details of your child's",
      15,
      20
    );
    doc.text(
      `performance in the ${cls?.course_name} Education Center. Please find below the information regarding their progress and `,
      15,
      25
    );
    doc.text(
      "achievements. If you have any questions or need further assistance, do not hesitate to contact us.",
      15,
      30
    );
  ...
  return (
    <>
      <p className="px-14 mb-4 font-normal">
        Welcome to Class{" "}
        <span className="font-bold capitalize">{params?.classHandle}</span>
      </p>
      <div
        onClick={closePopUp}
        className="px-2 sm:px-10 bg-gray-50 w-full h-full min-h-[calc(100vh-150px)] relative"
      >
        <Notification
        ...
        {showPopUp && (
          <PopUp
            data={students}
            showEditModal={handleRowDoubleClick}
        ...
        {showFullModal && (
          <FullModal showModal={true} handleClose={handleCloseModal}>
            {isEditable && pageWillShow == "updateStudent" ? (
              <UpdateStudents data={singleStudent} />
            ) : (
              <AddStudent data={singleStudent} />
            )}
          </FullModal>
        )}
          ...
          <div>
            <div className="hidden md:block">
              <Table
                ...
                bodyRows={students?.map((cls: any, index: number) => [
                  <div
                    onClick={() => handleRowDoubleClick(cls._id)}
                    key={index}
                  ...
                  <>
                    {emailsSent.has(cls._id) ? (
                      <p className="text-gray-500 ">
                        You already sent the email
                      </p>
                    ) : (
                      <div className="flex gap-x-4 items-center justify-center">
                        <div
                          className="cursor-pointer"
                          onClick={() => handleConfirmation(cls, "present")}
                        >
                          <PresentIcon />
                        </div>
                        <div
                          className="cursor-pointer"
                          onClick={() => handleConfirmation(cls, "absent")}
                        >
                          <AbsentIcon />
                        </div>
                      </div>
            ...
            <div className="block md:hidden my-5">
              {students?.map((cls: any) => (
                <TableForMobileScreen
                  key={cls._id}
                  data={cls}
                  isStudent={true}
                  updatedClass={updateStudent}
                  handleShowPopUp={editStudent}
                />
              ))}
            </div>
          </div>
      ...
      {showReportModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white p-6 rounded-lg">
            ...
            <p>
              Are you sure you want to send the report email to{" "}
