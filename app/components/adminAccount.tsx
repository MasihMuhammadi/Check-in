...
const AdminAccount = ({ data }: { data: any }) => {
  ...
  const logoutUser = async () => {
    try {
      await axios.delete(
        `${baseUrl}/api/public-courses/p-courses/${privateId}`,
        { withCredentials: true }
      );
      await axios.delete(`${baseUrl}/api/courses/course/${data?._id}`, {
        withCredentials: true,
      });
  ...
  return (
    <>
      <div className="flex flex-col items-center justify-center">
        ...
        <p>We are so sorry to lose you!!!</p>
        <div>
          <input type="checkbox" onChange={confirmDeletion} />
          If you want to delete your account, please check the box below
          <br />
        </div>
        <button
          disabled={disabledButton}
          className={`mt-5 px-3 py-2 rounded ${
            disabledButton ? "bg-red-300" : "bg-red-600"
          } text-white`}
