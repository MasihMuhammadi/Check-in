...
const CourseClases = ({
  ...
  const pageWillShow = useSelector(
    (state: any) => state.classSlice.pageWillShow
  );
  ...
  const handleUpdate = async (updatedClassData: any) => {
    try {
      await axios.put(
        `${baseUrl}/api/classes/class/${updatedClassData._id}`,
        updatedClassData
      );
      setClasses(
        classes.map((cls: any) =>
          cls._id === updatedClassData._id ? updatedClassData : cls
        )
      );
    } catch (error) {
        ...
        {classes?.length > 0 ? (
          classes?.map((cls: any, index: number) => {
            return (
              <div
                key={index}
                className="border shadow-xl rounded-2xl bg-gray-50 w-auto h-auto p-4 text-center"
              >
                <div
                  className="mb-4 block py-1"
                  onClick={() => showActionPopup(index)}
                >
                ...
                <p className="font-normal">
                  Start From <span className="font-bold">{cls?.start_day}</span>{" "}
                  to <span className="font-bold">{cls?.finish_day}</span>
                </p>
              </div>
            );
          })
          ...
          <div className="flex flex-col items-center">
            ...
            <NoCourseFound
              className={"mt-5 w-full sm:w-[450px] opacity-85 m-auto"}
            />
          </div>
        )}
      </div>
