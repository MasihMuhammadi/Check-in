    ...
    const getSingleCourse = async () => {
      try {
        const res = await axios.get(
          `${baseUrl}/api/public-courses/p-courses/${data?.handle}`,
          { withCredentials: true }
        );
        const fetchedData = res.data;
        setIsDataExist(fetchedData);
        const base64Image: any = Buffer.from(
          fetchedData.Images,
          "binary"
        ).toString("base64");
    ...
    try {
      ...
      if (!isDataExist?.location) {
        ...
        response = await axios.put(
          `${baseUrl}/api/public-courses/p-courses/${isDataExist._id}`,
          formData,
          {
            withCredentials: true,
            headers: {
      ...
      setImage(
        Buffer.from(updatedCourse.data.Images, "binary").toString("base64")
      );
        ...
        <Form className="overflow-x-hidden" onSubmit={handleSubmit}>
          ...
          {isLoading ? (
            <div className="flex flex-col items-center justify-center">
              <Skeleton
                width={200}
                className="mt-2 h-40 items-center justify-center"
                count={1}
              />
              <Skeleton
                width={400}
              ...
              <div className="relative mt-5">
                <div className="flex justify-center gap-x-4 items-center">
                  <Field
                    ...
                    disabled={true}
                    name="isVisable"
                    value={values.isVisable}
                    className="w-10 p-5 text-red-600"
                    id="isVisible"
                  />
                  <label htmlFor="isVisable" className="text-gray-400">
                    Do you want to preview your course details on our{" "}
