...
const AllClassesTab = ({ data, params }: { data: any; params: any }) => {
  ...
  useEffect(() => {
    const getClasses = async () => {
      setIsLoading(true);
      try {
        const response: any = await axios.get(`${baseUrl}/api/classes/class`);
        let allClasses = response?.data?.data;

        const desiredClasses = allClasses.filter(
          (cls: any) =>
            cls.teacher_name ===
            singleTeacherData?.data?.data?.teacher?.teacher_name
        );
  ...
  const deleteClassById = async (id: any) => {
    if (id) {
      try {
        await axios.delete(`${baseUrl}/api/classes/class/${id}`, {
          withCredentials: true,
        });
        setFilteredClasses((prevClasses) =>
          prevClasses.filter((cls: any) => cls._id !== id)
        );
      } catch (error) {
  ...
  const handleSearch = () => {
    setIsSearched(true);
    const result = filteredClasses.filter((course: any) =>
      course.class_name.toLowerCase().includes(searchValue.toLowerCase())
    );
  ...
  const classToDisplay =
    searchedClass.length > 0 ? searchedClass : filteredClasses;
    ...
    <div className="relative">
      {(showFullModal || isEditable) && pageToShow === "addaClass" && (
        <FullModal
          ...
          showModal={showFullModal}
          setShowModal={setShowFullModal}
        >
          <div className="flex ">
            {isEditable ? (
              <UpdateClass data={updatedClass} />
            ) : (
              <AddClass data={data} />
            )}
          </div>
      ...
      <div className=" ">
        ...
        <Buttons
          type="button"
          secondary={true}
          clickHandler={handleFullModal}
          style="px-5 mt-10 ml-10"
        >
